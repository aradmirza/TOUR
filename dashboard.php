...
// --- Next upcoming / currently running tour ---
$nextTour = null;
foreach ($groups as $g) {
...
<div class="hero-countdown">
  <?php if ($nextTour['cover_photo'] && file_exists(UPLOAD_GROUP . $nextTour['cover_photo'])): ?>
    <div class="hero-countdown-img" style="background-image:url('uploads/group/<?= e($nextTour['cover_photo']) ?>')"></div>
  ...
  <div class="hero-countdown-content">
    <div class="hero-badge">
      <?= $status === 'running' ? '🟢 TOUR IS RUNNING' : '⏳ UPCOMING TOUR' ?>
    </div>
    ...
    <?php if ($status === 'upcoming'): ?>
      <div class="countdown-widget" id="countdown"
           data-start="<?= e($nextTour['start_date']) ?>">
    ...
    <?php else: ?>
      <div class="countdown-status countdown-running">🟢 Tour in progress — <?= e($nextTour['destination']) ?></div>
    <?php endif; ?>
    <div style="margin-top:16px;">
      <a href="group.php?id=<?= $nextTour['id'] ?>" class="btn btn-outline" style="color:#fff;border-color:rgba(255,255,255,0.5);">
        View Group →
      </a>
    </div>
  </div>
      ...
      <a href="group.php?id=<?= $g['id'] ?>" class="group-card" style="display:block;margin-bottom:12px;text-decoration:none;">
        <div class="group-card-cover">
          ...
          <div class="group-card-status">
            <span class="badge badge-<?= $s === 'upcoming' ? 'blue' : ($s === 'running' ? 'green' : 'gray') ?>">
              <?= ucfirst($s) ?>
            </span>
          </div>
        </div>
      ...
      <div class="post-card">
        <div class="post-header">
          <div class="post-author">
            <?= avatarHtml($post['author_name'], $post['author_photo'], 36) ?>
